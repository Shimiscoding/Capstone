...
@php
    $sectionTitle = match ($userSection) {
        'supervisors' => 'Supervisors',
        'enforcers' => 'Enforcers',
        'admins' => 'Admins',
    };
    $sectionRoute = match ($userSection) {
    ...
    $managementSubject = match ($userSection) {
        'supervisors' => 'Supervisor',
        'enforcers' => 'Enforcer',
        'admins' => 'Admin',
    };
    $managementDescription = match ($userSection) {
    ...
    $shownFilters = array_intersect((array) request()->query('filters', []), ['joined']);
    $showJoinedFilter = $joinedFrom !== '' || $joinedTo !== '' || in_array('joined', $shownFilters, true);
@endphp
@section('title', $sectionTitle)
@section('activePage', 'users-' . $userSection)
...
@section('content')
    ...
    <form id="userTableFilters" class="user-table-filters users-reference-toolbar" method="GET" action="{{ $sectionRoute }}" role="search">
        <div class="users-toolbar-main">
            <div class="users-toolbar-actions">
                ...
                <a class="toolbar-outline-button toolbar-add-user" href="{{ match ($userSection) {'supervisors' => route('dashboard.users.supervisors.create'),'enforcers' => route('dashboard.users.enforcers.create'),'admins' => route('dashboard.users.admins.create')} }}">Add User<svg viewBox="0 0 16 16" aria-hidden="true"><path d="m4 6 4 4 4-4"/></svg></a>
            </div>
        </div>
    ...
    <div class="table-card" aria-live="polite"><div class="table-scroll"><table class="users-table">
        ...
        @forelse ($users as $listedUser)
            ...
            <tr>
                <td><div class="user-cell"><span class="table-avatar">{{ $listedInitials ?: 'U' }}</span><span><strong><a class="user-name-link" href="{{ match ($userSection) {'supervisors' => route('dashboard.users.supervisors.show', $listedUser),'enforcers' => route('dashboard.users.enforcers.show', $listedUser),'admins' => route('dashboard.users.admins.show', $listedUser)} }}">{{ $listedName }}</a></strong></span></div></td>
                <td class="location-cell"><strong>{{ $listedUser->area ?: '—' }}</strong><small class="location-truncate">{{ $listedUser->address ?: 'No address' }}</small></td>
                <td>{{ $listedUser->phoneNumber ?: '—' }}</td><td class="user-role-text">{{ $listedUser->isOfficer() ? 'Enforcer' : ucfirst($listedUser->role) }}</td><td>{{ $listedUser->created_at?->format('M d, Y') ?? '—' }}</td>@php $accountStatus = $listedUser->effectiveAccountStatus(); @endphp<td><span class="status-badge status-{{ $accountStatus }}" title="{{ match ($accountStatus) {'active' => 'Able to log in.','pending' => 'Waiting for administrator activation.','inactive' => 'Deactivated or unused for a long time.','suspended' => 'Temporarily blocked.','banned' => 'Permanently blocked.'} }}"><i></i> {{ ucfirst($accountStatus) }}</span></td>
                <td><div class="user-row-actions"><details class="user-actions-menu"><summary aria-label="Actions for {{ $listedName }}" title="Actions">&#8942;</summary><div class="user-actions-dropdown"><a class="user-action action-edit" href="{{ route('dashboard.users.edit', ['user' => $listedUser, 'from' => $userSection]) }}">Edit</a><form method="POST" action="{{ route('dashboard.users.destroy', $listedUser) }}" onsubmit="return confirm('Delete this user? This cannot be undone.')">@csrf @method('DELETE')<input type="hidden" name="user_section" value="{{ $userSection }}"><button class="user-action action-delete" type="submit">Delete</button></form></div></details></div></td>
            </tr>
        @empty
