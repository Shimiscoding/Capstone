...
@section('headerSearch')
  <form class="search" method="GET" action="{{ route('dashboard.users') }}" role="search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg><input type="search" name="search" value="{{ $search }}" placeholder="Search users">@if ($role !== '')<input type="hidden" name="role" value="{{ $role }}">@endif<input type="hidden" name="sort" value="{{ $sort }}"></form>
@endsection

@section('content')
  <div class="page-head users-page-head"><div><p class="eyebrow">User management</p><h1>Users</h1><p class="page-description">View all registered TOMECO accounts and their contact details.</p></div><div class="users-page-actions"><div class="user-count">{{ number_format($users->total()) }} {{ \Illuminate\Support\Str::plural('user', $users->total()) }}</div><a class="add-user-button" href="{{ route('dashboard.users.create') }}">+ Add user</a></div></div>
  ...
  <form id="userTableFilters" class="user-table-filters" method="GET" action="{{ route('dashboard.users') }}" role="search">
    <label class="user-filter-search"><span>Search by name</span><input id="userNameSearch" type="search" name="search" value="{{ $search }}" placeholder="Enter a users name" autocomplete="off"></label>
    <label><span>Filter by role</span><select id="userRoleFilter" name="role"><option value="" @selected($role === '')>All roles</option><option value="driver" @selected($role === 'driver')>Drivers only</option><option value="officer" @selected($role === 'officer')>Enforcers only</option><option value="admin" @selected($role === 'admin')>Admins only</option></select></label>
    ...
    <button type="submit">Search</button><a href="{{ route('dashboard.users') }}">Clear filters</a>
  </form>

  <div class="table-card" aria-live="polite"><div class="table-scroll"><table class="users-table">
    <thead><tr><th scope="col">User</th><th scope="col">Badge number</th><th scope="col">Phone number</th><th scope="col">Plate number</th><th scope="col">Role</th><th scope="col">Joined</th><th scope="col">Status</th><th scope="col">Actions</th></tr></thead>
    <tbody>
      @forelse ($users as $listedUser)
        ...
        <tr>
          <td><div class="user-cell"><span class="table-avatar">{{ $listedInitials ?: 'U' }}</span><span><strong>{{ $listedName }}</strong><small>{{ $listedUser->email }}</small></span></div></td>
          <td><span class="badge-number">{{ $listedUser->badgeNumber ?: '—' }}</span></td><td>{{ $listedUser->phoneNumber ?: '—' }}</td><td>{{ $listedUser->plateNumber ?: '—' }}</td><td><span class="role-badge role-{{ $listedUser->role }}">{{ ucfirst($listedUser->role) }}</span></td><td>{{ $listedUser->created_at?->format('M d, Y') ?? '—' }}</td><td><span class="status-badge"><i></i> Registered</span></td>
          <td><div class="user-row-actions" aria-label="Actions for {{ $listedName }}"><a class="user-action action-edit" href="{{ route('dashboard.users.edit', $listedUser) }}">Edit</a><form method="POST" action="{{ route('dashboard.users.destroy', $listedUser) }}" onsubmit="return confirm('Delete this user? This cannot be undone.')">@csrf @method('DELETE')<button class="user-action action-delete" type="submit">Delete</button></form></div></td>
        </tr>
      @empty
        <tr><td colspan="8" class="empty-state"><strong>{{ $search !== '' ? 'No users found' : 'No registered users yet' }}</strong><span>{{ $search !== '' ? 'Try a different name.' : 'Newly registered accounts will appear here.' }}</span></td></tr>
      @endforelse
    </tbody>
  </table></div>
...
<script>
  ...
  async function updateUserTable(url, pushState = true) {
    ...
    try {
      ...
      nextTable.classList.add('is-entering'); currentTable.replaceWith(nextTable); document.querySelector('.user-count').replaceWith(nextCount); userNameSearch.value = page.getElementById('userNameSearch')?.value ?? ''; document.getElementById('userRoleFilter').value = page.getElementById('userRoleFilter')?.value ?? ''; document.getElementById('userSort').value = page.getElementById('userSort')?.value ?? 'latest'; requestAnimationFrame(() => nextTable.classList.remove('is-entering')); if (pushState) history.pushState({}, '', url);
